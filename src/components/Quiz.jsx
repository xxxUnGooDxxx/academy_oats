...
// Индексы в `correct` пересчитываются под новый порядок.
function shuffleQuestions(questions) {
  return questions.map((q) => {
    ...
    const correct = Array.isArray(q.correct) ? q.correct.map(remap) : remap(q.correct)
...
export default function Quiz({ questions, onFinish, onStatusChange, title = 'Проверка' }) {
  const [items, setItems] = useState(() => shuffleQuestions(questions))
  ...
  function restart() {
    setItems(shuffleQuestions(questions))
    setCurrent(0)
    setSelected([])
    setChecked(false)
    setScore(0)
